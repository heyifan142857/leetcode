...
public:
    int maxStability(int n, vector<vector<int>> &edges, int k) {
        // Preliminary checks: must-edges should not form a cycle, and graph
        // must be connected
        UnionFind ufCheck(n);
        ...
        if (ufAll.components() != 1)
            return -1; // graph not connected at all

        long long maxS = 0;
        for (const auto &e : edges)
            if (e.size() >= 4)
                maxS = max(maxS, (long long)e[2]);
