...
public:
    int maxProductPath(vector<vector<int>> &grid) {
        ...
        auto better = [&](const State &a, const State &b) -> bool {
            ...
            if (a.isNeg == b.isNeg) {
                if (!a.isNeg)
                    return a.logAbs > b.logAbs; // larger positive
                return a.logAbs < b.logAbs;     // for negatives, smaller abs is
                                                // larger numerically
            }
        ...
        auto worse = [&](const State &a, const State &b) -> bool {
            ...
            if (a.isNeg == b.isNeg) {
                if (a.isNeg)
                    return a.logAbs > b.logAbs; // both negative: larger abs ->
                                                // more negative -> smaller
                return a.logAbs <
                       b.logAbs; // both positive: smaller abs -> smaller
            }
            return a.isNeg; // if a negative and b positive, a < b
        };

        vector<vector<State>> dpMax(m, vector<State>(n)),
            dpMin(m, vector<State>(n));
        ...
        auto makeStateFromValue = [&](int v) -> State {
            ...
            if (v == 0) {
                ...
                s.isZero = false;
                s.isNeg = (v < 0);
                s.logAbs = logl((long double)llabs((long long)v));
                s.modAbs = (llabs((long long)v)) % MOD;
            }
            return s;
        };
            ...
            for (int j = 0; j < n; ++j) {
                ...
                auto processPred = [&](const State &pred) {
                    ...
                    if (pred.isZero || v == 0) {
                        cand.isZero = true;
                        cand.isNeg = false;
                        cand.logAbs = -INFINITY;
                        cand.modAbs = 0;
                    } else {
                        cand.isZero = false;
                        cand.isNeg = pred.isNeg ^ (v < 0);
                        cand.logAbs = pred.logAbs +
                                      logl((long double)llabs((long long)v));
                        cand.modAbs =
                            (pred.modAbs * (llabs((long long)v) % MOD)) % MOD;
                    }

                    if (!bestMax.valid || better(cand, bestMax))
                        bestMax = cand;
                    if (!bestMin.valid || worse(cand, bestMin))
                        bestMin = cand;
                };
