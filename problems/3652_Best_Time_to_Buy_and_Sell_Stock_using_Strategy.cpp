...
public:
    long long maxProfit(vector<int> &prices, vector<int> &strategy, int k) {
        ...
        // prefix_strategy_price[i] = sum of strategy[j] * prices[j] from 0 to
        // i-1 prefix_price[i] = sum of prices[j] from 0 to i-1
        vector<long long> prefix_strategy_price(n + 1, 0);
        vector<long long> prefix_price(n + 1, 0);

        for (int i = 0; i < n; i++) {
            prefix_strategy_price[i + 1] =
                prefix_strategy_price[i] + (long long)strategy[i] * prices[i];
        ...
        for (int i = 0; i <= n - k; i++) {
            ...
            change -= (prefix_strategy_price[firstHalfEnd] -
                       prefix_strategy_price[i]);

            // Last k/2 elements: change from strategy[j] to 1 (sell)
            // Change = sum((1 - strategy[j]) * prices[j]) for j in [i + k/2, i
            // + k) = sum(prices[j]) - sum(strategy[j] * prices[j])
            int secondHalfStart = i + k / 2;
            int secondHalfEnd = i + k;
            change +=
                (prefix_price[secondHalfEnd] - prefix_price[secondHalfStart]) -
                (prefix_strategy_price[secondHalfEnd] -
                 prefix_strategy_price[secondHalfStart]);
