        ...
        for (const auto num : nums) {
            if (num % 2 == 0) {
                ...
                if (temp == 0) {
                    ...
                    // x should have k-1 trailing ones, then 0, then same upper
                    // bits as n Formula: keep upper bits, set trailing k bits
                    // to: (2^(k-1) - 1)
                    int mask =
                        (1 << trailing_ones) - 1; // mask for trailing k bits
                    ...
                    if ((x | (x + 1)) == num) {
                        ...
                        // Fallback: search for minimum x (shouldn't happen with
                        // correct formula)
                        int min_x = -1;
