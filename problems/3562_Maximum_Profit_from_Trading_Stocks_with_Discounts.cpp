...
public:
    int maxProfit(int n, vector<int> &present, vector<int> &future,
        ...
        // Memoization: node -> pair of (dp when parent didn't buy, dp when
        // parent bought)
        map<int, pair<vector<int>, vector<int>>> memo;

        // DFS function: returns both dp arrays (when parent didn't buy, when
        // parent bought)
        function<pair<vector<int>, vector<int>>(int)> dfs =
            [&](int node) -> pair<vector<int>, vector<int>> {
            if (memo.count(node)) {
                return memo[node];
            }
            ...
            // Helper function to merge children's DP arrays (optimized
            // knapsack)
            auto mergeChildren =
                [&](const vector<vector<int>> &childDps) -> vector<int> {
                if (childDps.empty()) {
                ...
                for (int i = 1; i < childDps.size(); i++) {
                    ...
                    for (int b = budget; b >= 0; b--) {
                        for (int c = 0; c <= b; c++) {
                            newMerged[b] = max(newMerged[b],
                                               merged[b - c] + childDps[i][c]);
                        }
                    }
                    merged = newMerged;
                }
            ...
            for (const auto &childRes : childResults) {
                childDpsNoDiscount.push_back(
                    childRes.first); // parent didn't buy
                childDpsWithDiscount.push_back(
                    childRes.second); // parent bought
            }

            // Merge children once for each scenario (reuse results)
            vector<int> mergedNoDiscount = mergeChildren(childDpsNoDiscount);
            vector<int> mergedWithDiscount =
                mergeChildren(childDpsWithDiscount);
            ...
            for (int b = 0; b <= budget; b++) {
                ...
                if (b >= costWithoutDiscount) {
                    profit2 = profitWithoutDiscount +
                              mergedWithDiscount[b - costWithoutDiscount];
                }
            ...
            for (int b = 0; b <= budget; b++) {
                ...
                if (b >= costWithDiscount) {
                    profit2 = profitWithDiscount +
                              mergedWithDiscount[b - costWithDiscount];
                }
